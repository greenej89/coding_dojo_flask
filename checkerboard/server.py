...
@app.route('/<int:num>')
def square_checkerboard(num,color1="red",color2="black"):
    return render_template('index.html',x=num,y=num,color1=color1,color2=color2)
...
def rectangular_checkerboard(x,y,color1="red",color2="black"):
    return render_template('index.html',x=x, y=y,color1=color1,color2=color2)
...
def colorful_checkerboard(x,y,color1,color2):
    return render_template('index.html',x=x, y=y,color1=color1,color2=color2)

if __name__ == "__main__":
    app.run(debug=True)
    # app.run(debug=True, port= 5001) #For Mac owners who like Google Chrome